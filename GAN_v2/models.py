...
class ConvGenerator(nn.Module):
    ...
    def __init__(self, latent_dim, ts_dim, hidden_dim=256, hidden_dim2=10):
        ...
        self.shifting_block2 = nn.Sequential(
            nn.Linear(hidden_dim2 * latent_dim, hidden_dim),
            nn.LeakyReLU(inplace=True))

        self.fc = nn.Sequential(nn.Linear(hidden_dim, ts_dim))
    ...
    def forward(self, x):
        ...
        out4 = torch.flatten(out4, start_dim=1)
        out4 = self.shifting_block2(out4)
...
if __name__ == '__main__':
    input = torch.randn(1, 64, 800)

    Dis = LSTMDiscriminator(ts_dim=800)
    out = Dis(input)
    print("out: ", out.size())
    print("**********************************")
    input = torch.randn(1, 64, 50)
    Gen = LSTMGenerator(latent_dim=50, ts_dim=800)
    out = Gen(input)
    print("out: ", out.size())
